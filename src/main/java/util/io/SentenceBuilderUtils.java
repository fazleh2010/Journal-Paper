...
import grammar.generator.helper.datasets.sentencetemplates.TemplateVariable;
import static grammar.generator.helper.datasets.sentencetemplates.TemplateVariable.colon;
import static grammar.generator.helper.datasets.sentencetemplates.TemplateVariable.defaultGender;
import static grammar.generator.helper.datasets.sentencetemplates.TemplateVariable.directObject;
import static grammar.generator.helper.datasets.sentencetemplates.TemplateVariable.domain;
import static grammar.generator.helper.datasets.sentencetemplates.TemplateVariable.gender;
import static grammar.generator.helper.datasets.sentencetemplates.TemplateVariable.range;
...
public class SentenceBuilderUtils implements TemplateConstants {
   ...
   /* private LexicalEntryUtil lexicalEntryUtil = null;
    ...
    public String getWords(String[] tokens, Integer index) throws QueGGMissingFactoryClassException {
        ...
        } else if (flagReference && isInterrogativePlacenWord(attribute).first) {
            SubjectType subjectType = isInterrogativePlacenWord(attribute).second;
            if (reference.contains(colon)) {
                String[] col = reference.split(colon);
                word = this.getDeteminerToken(subjectType, col[0], col[1]);
    ...
    public static Pair<Boolean, SubjectType> isInterrogativePlacenWord(String questionType) throws QueGGMissingFactoryClassException {
